...
#cp ~/Documents/.kulog ~/.
    ...
    rm -r ${ScriptDownloadPath}/"Clover Nightly Build"
...
if [ $kexte = "2" ]; then
    kextLoadArray=("fakesmc" "opencore" "usbinjectall" "voodoops2")
    recho="Kexte"
    _main "kextLoader"
    exit 0
fi
...
if [ $kexte = "3" ]; then
    kextLoadArray=("applealc" "codeccommander" "lilu")
    recho="Kexte"
    _main "kextLoader"
    exit 0
fi
...
if [ $kexte = "5" ]; then
    kextLoadArray=("atherose2200ethernet" "intelmausi" "realtekrtl8111")
    recho="Kexte"
    _main "kextLoader"
    exit 0
fi
...
if [ $kexte = "8" ]; then # Load Bootloader
...
../bin/./PlistBuddy -c Print "${ScriptHome}/Library/Preferences/com.slsoft.kextupdater.plist"| grep "dl-" | sort | xargs | sed -e "s/dl-/\"/g" -e "s/\ =\ true/\"/g" > "$ScriptTmpPath"/array
...
function checkatheros40()
{
    if [[ -d /Library/Extensions/AirPortAtheros40.kext ]] & [[ -d /Library/Extensions/IO80211Family.kext ]]; then
        defaults write "${ScriptHome}/Library/Preferences/com.slsoft.kextupdater.plist" "Atheros40" -bool YES
    else
        defaults write "${ScriptHome}/Library/Preferences/com.slsoft.kextupdater.plist" "Atheros40" -bool NO
    fi
}
